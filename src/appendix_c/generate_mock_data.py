...
Builds a 12-month Z-axis so the Three.js surface has visible ridges.
Nov 2025 to Jan 2026 use Appendix C.9 paper totals; later months are
synthetic fill for topology only (not an extended corpus claim).
"""
...
DEFAULT_OUT_DIR = REPO_ROOT / "mock_data"

# Appendix C.9 published corpus totals (pinned into the demo months).
PAPER_C9_MONTHS = {
    "2025-11": {
        "sessions": 80,
        "messages": 1354,
...
def build_monthly_mock_data(seed: int = 42) -> dict:
    """
    Build a 12-month series so the Three.js surface has enough Z-samples.

    Nov 2025 to Jan 2026 use Appendix C.9 paper totals. Later months are
    synthetic fill for topology only (not claimed as corpus).
    """
    ...
    for i, month in enumerate(months, start=1):
        if month in PAPER_C9_MONTHS:
            result[month] = dict(PAPER_C9_MONTHS[month])
        else:
            x = (i - 1) / max(1, total - 1)
        ...
        progress("Monthly series", i, total)
    print()
    return result


def build_tag_baselines(seed: int = 1337) -> dict:
    """Paper Appendix C tag baselines (exact percentages, no jitter)."""
    _ = seed
...
def main() -> None:
    ...
    payload = {
        "version": "mock-1.0",
        "description": (
            "Synthetic demo topology (12 months). Nov 2025 to Jan 2026 use Appendix C.9 "
            "paper totals; Feb 2026 to Oct 2026 are synthetic fill for ridges only. "
            "Not an extended corpus claim. No private transcripts."
        ),
    ...
    pages_mock = REPO_ROOT / "docs" / "demo" / "mock_data" / "appendix_c_mock_data.json"
    try:
        pages_mock.parent.mkdir(parents=True, exist_ok=True)
        pages_mock.write_text(out_path.read_text(encoding="utf-8"), encoding="utf-8")
        log("INFO", "generate_mock_data.main", "Synced Pages mock copy", pages_mock=str(pages_mock))
    except OSError as exc:
        log("WARN", "generate_mock_data.main", "Could not sync Pages mock copy", error_message=str(exc))
