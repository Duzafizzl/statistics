...
DEFAULT_TAXONOMY = REPO_ROOT / "config" / "appendix_c_taxonomy.yaml"
DEFAULT_OUTPUT = REPO_ROOT / "data" / "appendix_c_surface_data.json"


def path_for_payload(path: Path | None, repo_root: Path = REPO_ROOT) -> str:
    """Store repo-relative paths only; never leak absolute home directories."""
    if path is None:
        return ""
...
def main() -> None:
    ...
    parser.add_argument("--taxonomy", default=str(DEFAULT_TAXONOMY), help="Path to taxonomy YAML")
    parser.add_argument("--sessions", default="", help="Optional: path to session JSONL")
    parser.add_argument("--output", default=str(DEFAULT_OUTPUT), help="Output JSON path")
    args = parser.parse_args()

    # Resolve relative paths against the caller's CWD (usually repo root).
    if args.taxonomy and not Path(args.taxonomy).is_absolute():
        args.taxonomy = str((Path.cwd() / args.taxonomy).resolve())
    if args.output and not Path(args.output).is_absolute():
        args.output = str((Path.cwd() / args.output).resolve())
    if args.sessions and not Path(args.sessions).is_absolute():
        args.sessions = str((Path.cwd() / args.sessions).resolve())
    ...
    try:
        ...
        if source == "sessions_jsonl":
            description = (
                "Surface baselines aggregated from optional session JSONL plus taxonomy. "
                "Private transcripts are not shipped in this repository."
            )
        else:
            description = (
                "Surface baselines from taxonomy YAML only (no session JSONL). "
                "Not a claim that private logs were processed in this build."
            )
        payload = {
            ...
            "source": source,
            "taxonomy_path": path_for_payload(taxonomy_path),
            "sessions_path": path_for_payload(sessions_path),
            "statistical_reference": {
                "monthly_baseline": monthly,
                "tag_baselines": tags,
            },
        ...
        # Keep Pages demo data in sync when writing the default output.
        pages_data = REPO_ROOT / "docs" / "demo" / "data" / "appendix_c_surface_data.json"
        if out_path.resolve() == DEFAULT_OUTPUT.resolve():
            try:
                pages_data.parent.mkdir(parents=True, exist_ok=True)
                pages_data.write_text(out_path.read_text(encoding="utf-8"), encoding="utf-8")
                slog("INFO", "build_surface_dataset.main", "Synced Pages surface copy", pages_data=str(pages_data))
            except OSError as exc:
                slog("WARN", "build_surface_dataset.main", "Could not sync Pages surface copy", error_message=str(exc))
        print()
